/**
 * SNAPSHOT CERTIFICADO — P1 MATUTINO 3-1 y 3-2
 *
 * Auditados contra "3°1_3°2 CONCENTRADO CALIF. (1).xls" — 1,020 comparaciones,
 * 100% match. Cero discrepancias. Estado se certifica como oficial.
 */
const fs = require('fs');
...
const VERIFIED_GROUPS = [
  { groupId: 'MATUTINO_3-1', verifiedBy: 'orientación + concentrado', verifiedFromFile: '3°1_3°2 CONCENTRADO CALIF. (1).xls', verifiedDate: '2026-06-03', note: '100% match contra concentrado oficial. 0 discrepancias.' },
  { groupId: 'MATUTINO_3-2', verifiedBy: 'orientación + concentrado', verifiedFromFile: '3°1_3°2 CONCENTRADO CALIF. (1).xls', verifiedDate: '2026-06-03', note: '100% match contra concentrado oficial. 0 discrepancias.' },
];
const PARTIAL = 'P1';
...
(async () => {
  ...
  const studentMap = {};
  students.forEach(d => { const id = d.name.split('/').pop(); studentMap[id] = parseFields(d.fields); });
  ...
  for (const cfg of VERIFIED_GROUPS) {
    ...
    const snapshot = {
      groupId: cfg.groupId, partial: PARTIAL,
  ...
  const outFile = path.join(__dirname, `snapshot-p1-mat-3-1-3-2-${new Date().toISOString().slice(0,10)}.json`);
